<?php 
    ...
    $calle = $pdo->prepare("SELECT * FROM usuarios inner join reportes WHERE reportes.id = :id and reportes.tipo_reporte = 1 and usuarios.cedula = reportes.id_cedula;");
...
<body>
    <div style="margin-top: 150px;" class="container">
        <form class="form" action="actualizar-calle.php" method="POST">
            ...
            <input type="hidden" name="id" value="<?= openssl_encrypt($calle['id'], AES, KEY) ?>">
            <!-- <input type="text" name="tipo" placeholder="Tipo" value="<?php // $calle['tipo_reporte'] ?>"> -->
            <div class="form__label">
                <label for="">Descripción:</label>
                <input type="text" name="descripcion" placeholder="Descripcion" value="<?= $calle['descripcion'] ?>">
            </div>
            <div class="form__label">
                <label for="">Ubicación:</label>
                <input type="text" name="ubicacion" placeholder="Ubicacion" value="<?= $calle['ubicacion'] ?>">
            </div>
            <div class="form__label">
                <label for="">Fecha:</label>
                <input type="datetime" name="fecha" placeholder="Fecha" value="<?= $calle['fecha'] ?>">
            </div>
