<?php 
    ...
    function cambiarContrasena($pdo, $cedula, $password): void{
        try{
            $password = password_hash($password, PASSWORD_BCRYPT);
            $query = $pdo->prepare("UPDATE usuarios SET contrasenia = :pass WHERE cedula = :cedula");
            $query->bindParam(':pass', $password);
            $query->bindParam(':cedula', $cedula);
            $query->execute();

            $_SESSION["success"] = "Se cambio la contraseña correctamente";
            $_SESSION['verificar'] = null;
            $_SESSION['error'] = null;
            header('location: login.php');
            exit();
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        //expresion regular para validar la contraseña que contenga al menos una letra mayuscula, una minuscula, un numero y un caracter especial
        if(!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]{8,}$/', $password)){
            $_SESSION["error"] = "La contraseña debe contener al menos una letra mayuscula, una minuscula, un numero y un caracter especial";
            header('location: reset-password.php');
            exit();
        }

        cambiarContrasena($pdo, $_SESSION['verificar'], $password);
    }
?>
...
<head>
    ...
    <title>SySReport - Reset Password </title>
...
?> -->
<body style="display: flex; align-items: center; height: 100vh; flex-direction: column; justify-content: center;">
    <div class="container">
        <form class="form" action="" method="POST">
            ...
            <div class="form__label">
                <label for="username" class="">Nueva contraseña</label>
                <input placeholder="Ingrese su nueva Contraseña" type="password" id="username" name="password-new" class="">
            </div>
            <div class="form__label">
                <label for="username" class="">Confirmar contraseña</label>
                <input placeholder="Confirme su nueva contraseña" type="password" id="username" name="password-confirm" class="">
            </div>
            <button type="submit" id="button" class="">Guardar</button>
        </form>
    </div>
</body>
</html>