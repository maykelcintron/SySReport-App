<?php 
    ...
    $id = urlencode(openssl_decrypt(htmlspecialchars($_GET['id']), AES, KEY));

    $tuberia = $pdo->prepare("SELECT * FROM usuarios inner join reportes WHERE reportes.id = :id and reportes.tipo_reporte = 2 and usuarios.cedula = reportes.id_cedula;");
    $tuberia->bindParam(':id', $id);
    $tuberia->execute();
    $tuberia = $tuberia->fetch(PDO::FETCH_ASSOC);
?>
...
<body>
    <div style="margin-top: 150px;" class="container container--modifier">
        <form class="form" action="actualizar-tuberia.php" method="POST">
            ...
            <div class="form__label">
                <input type="hidden" name="id" value="<?= openssl_encrypt($tuberia['id'], AES, KEY) ?>">
            </div>
            <div class="form__label">
                <label for="">Descripcion</label>
                <input type="text" name="descripcion" placeholder="Descripcion" value="<?= $tuberia['descripcion'] ?>">
            </div>
            <div class="form__label">
                <label for="">Ubicacion</label>
                <input type="text" name="ubicacion" placeholder="Ubicacion" value="<?= $tuberia['ubicacion'] ?>">
            </div>
            <div class="form__label">
                <label for="">Fecha</label>
                <input type="datetime" name="fecha" placeholder="Fecha" value="<?= $tuberia['fecha'] ?>">
            </div>
