<?php 
    ...
    if(isset($_POST['agregar'])){
        ...
        if(empty($descripcion) || empty($ubicacion)){
            $_SESSION['error_tuberia'] = 'Todos los campos son obligatorios para crear un reporte';
            header('Location: /infraestructura/tuberias/tuberias.php');
            exit;
        }

        $stmt = $pdo->prepare("INSERT INTO reportes (id_cedula, tipo_reporte, descripcion, ubicacion) VALUES (30490358, 2, :descripcion, :ubicacion)");
        $stmt->execute([
...
<body>
    <div style="margin-top: 150px;" class="container container--modifier">
        <form class="form" action="agregar-tuberia.php" method="POST">
