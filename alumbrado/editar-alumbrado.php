<?php 
    ...
    $alumbrado = $pdo->prepare("SELECT * FROM usuarios inner join reportes WHERE reportes.id = :id and reportes.tipo_reporte = 3 and usuarios.cedula = reportes.id_cedula;");
    $alumbrado->bindParam(':id', $id);
    $alumbrado->execute();
    $alumbrado = $alumbrado->fetch(PDO::FETCH_ASSOC);
?>
...
<body>
    <div style="margin-top: 150px;" class="container">
        <form class="form" action="actualizar-alumbrado.php" method="POST">
            ...
            <input type="hidden" name="id" value="<?= openssl_encrypt($alumbrado['id'], AES, KEY) ?>">

            <div class="form__label">
                <label for="">Descripción:</label>
                <input type="text" name="descripcion" placeholder="Descripcion" value="<?= $alumbrado['descripcion'] ?>">
            </div>

            <div class="form__label">
                <label for="">Ubicación:</label>
                <input type="text" name="ubicacion" placeholder="Ubicacion" value="<?= $alumbrado['ubicacion'] ?>">
            </div>

            <div class="form__label">
                <label for="">Fecha:</label>
                <input type="datetime" name="fecha" placeholder="Fecha" value="<?= $alumbrado['fecha'] ?>">
            </div>
